...
if(global.map_destination_name != global.map_curr_town_name) //TODO; saved popped tracks in order to move back
{
	...
	var track = ds_stack_top(global.stack_traj_front);
	
	if(track.track_dir == "LEFT" || track.track_dir == "RIGHT")
	ds_stack_pop(global.stack_traj_front);//remove it
	
	//get next track
	track = ds_stack_top(global.stack_traj_front);
	
	while(track.bordering_town == noone)
	{
		...
		track = ds_stack_pop(global.stack_traj_front);
				
	}//end while
	
	//if(track.bordering_town.town_name != global.map_destination_name)//while not arrived to dest
	//break;//break out of while if arrive at destination
